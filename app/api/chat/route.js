...
Generate the study plans with these strategies in mind, guiding the students to optimal studying. Kindly just provide the plan without explicitly stating that you used this method. 
...
async function generateNaturalResponse(history, pddlPlan) {
  ...
  const response = await groqClient.chat.completions.create({
    messages: [
      {
        role: 'system',
        content: `This study plan: ${pddlPlan} was generated by the fast downward planner in response to the prompt gotten from the users. Kindly convert into natural language so the users will understand it. . Do not add any introduction or explanation—just present the steps. `
      },
     ...history,
    ],
    model: 'llama3-70b-8192',
